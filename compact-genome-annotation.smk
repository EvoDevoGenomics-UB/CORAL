...
rule run_operon_finder_and_sanatizing:
    ...
    python {exeOpF} -f {input.gtf} --threshold {params.threshold} -o {params.name} --log {log}
...
rule run_final_operon_search:
    ...
    python {exeOpF} -f {input.gtf} --threshold {params.threshold} -o {output.dir_name}/{params.name} --log {log}
    """
