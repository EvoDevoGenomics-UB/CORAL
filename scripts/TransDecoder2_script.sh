...
scripts/TD2_util/gtf_genome_to_cdna_fasta.pl $GTF $GENOME > ${DIR}$NAME.fasta 

scripts/TD2_util/gtf_to_alignment_gff3.pl $GTF > ${DIR}$NAME.gff3
...
TD2.Predict -t ${DIR}$NAME.fasta -O ${DIR}$NAME --retain-mmseqs-hits ${DIR}$NAME/combined_alnRes.m8 #--retain_pfam_hits ${DIR}${NAME}.pfam.domtblout

scripts/TD2_util/cdna_alignment_orf_to_genome_orf.pl \
     ${DIR}$NAME.fasta.TD2.gff3 \
     ${DIR}$NAME.gff3 \
     ${DIR}$NAME.fasta > ${DIR}$NAME.fasta.TD2.genome.gff3

mkdir -p ${DIR}${NAME}_files
mv ${DIR}${NAME}/ ${DIR}${NAME}_files/${NAME}
mv ${DIR}${NAME}.fasta.TD2.gff3 ${DIR}${NAME}_files
