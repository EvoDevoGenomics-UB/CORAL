...
def validate_samplesheet(samples_df):
    ...
    for _, row in samples_df.iterrows():
        ...
        if not os.path.exists(fq):
            print(
                f"ERROR: Missing FASTQ file for sample '{sample}': {fq}",
                file=sys.stderr
            )
            sys.exit(1)
    ...
    for sample, fqs in grouped.items():
        if len(fqs) != len(set(fqs)):
            print(
                f"ERROR: Sample '{sample}' lists the same FASTQ file multiple times.",
                file=sys.stderr
            )
            sys.exit(1)
...
if "samplesheet" in config and config["samplesheet"]:
    ...
    SAMPLE_TO_FASTQ = {
        s: [f"{config['data_dir']}{s}{config['data_suffix']}"] for s in SAMPLES
    }


def get_gtf():
    gtf_input_list = []
    if config["extraGTF"] not in (None, [], ""):
        gtf_input_list = list(config["extraGTF"]) if isinstance(config["extraGTF"], list) else [config["extraGTF"]]

    if config["run_gffcompare"] == True:
        gtf_input_list.extend(
            expand("Gffcompare_results/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}_noOPRNs.annotated.gtf",
        ...
        gtf_input_list.extend(
            expand("annotations/{specie}/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}_StringtieMerge.clean-noOPRNs.gtf",
                specie=config["specie"],
                ref=config["stringtie_guide_opts"],
                intron=config["minimap2_max_intron"],
                threshold=config["operon_threshold"]
...
GTF_DICT = {os.path.basename(gtf).replace(".gtf", ""): gtf for gtf in get_gtf() if gtf}
...
def get_TD2_output():
    input_TD2_list = [
        expand("TD2_results/{specie}/{gtf_name}.fasta.TD2.genome.gff3",
            gtf_name=NAMES,
            specie=config["specie"]
        ),
        expand("busco_analysis/{specie}/{gtf_name}.TD2.pep.fasta",
            gtf_name=NAMES,
            specie=config["specie"]
        ),
        expand("busco_analysis/{specie}/BUSCO_prot_{gtf_name}",
            gtf_name=NAMES,
            specie=config["specie"]
        ),
    ]
    if config["reference_annot"] not in (None, [], ""):
        input_TD2_list.extend(
            expand("busco_analysis/{specie}/{specie}_LRannot_REF.pep.fasta",
                specie=config["specie"]
            )
        )
...
def get_expMatrix_output():
    expMatrix_input_list = [
        expand("Expression_matrix/{specie}/{gtf_name}/gene_count_matrix_v{intron}.csv",
            specie=config["specie"],
            intron=config["minimap2_max_intron"],
            gtf_name=NAMES
        )
...
def get_final_output():
    rule_all_input_list = [
        ...
        expand("GAMBA_results/{specie}/{specie}_{sample}_guide{ref}_v{intron}_opCLEAN_t{threshold}.clean.gtf",
            specie=config["specie"],
            sample=SAMPLES,
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
        ...
        expand("annotations/{specie}/Merge_OPRNs-OpGs_{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}.sorted.gtf",
            specie=config["specie"],
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
            threshold=config["operon_threshold"]
        ),
        expand("annotations/{specie}/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}_StringtieMerge.clean-andOPRNs.gtf",
            specie=config["specie"],
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
            threshold=config["operon_threshold"]
        ),
        expand("annotations/{specie}/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}_StringtieMerge.clean-noOPRNs.counts.gtf",
            specie=config["specie"],
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
            threshold=config["operon_threshold"]
        ...
        expand("busco_analysis/{specie}/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}_noOPRNs_longest_trans_only.fasta",
            specie=config["specie"],
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
            threshold=config["operon_threshold"]
        ),
        expand("busco_analysis/{specie}/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}_noOPRNs.fasta",
            specie=config["specie"],
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
            threshold=config["operon_threshold"]
        ),
        expand("busco_analysis/{specie}/{specie}_LRannot_guide{ref}_v{intron}_gambat{threshold}_andOPRNs.fasta",
            specie=config["specie"],
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
            threshold=config["operon_threshold"]
    ...
    if config["reference_annot"] not in (None, [], ""):
        rule_all_input_list.extend(
            expand("busco_analysis/{specie}/BUSCO_trans_{specie}_LRannot_REF",
                specie=config["specie"]
            )
        )
        if config["run_gffcompare"] == True:
