...
configfile:
    os.environ.get(
        "SNAKEMAKE_CONFIGFILE",
        path.join(path.dirname(workflow.snakefile), "CORAL-config.yaml"),
    )
...
rule dump_versions:
    ...
    conda:
        env_file
    shell:
        "command -v conda > /dev/null && conda list > {log}"
...
rule do_operon_annotations:
    input:
        expand("GAMBA_results/{specie}/{specie}_{sample}_guide{ref}_v{intron}_operons_found_t{threshold}.tsv",
            specie=config["specie"],
            sample=SAMPLES,
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
        ...
        expand("GAMBA_results/{specie}/{specie}_{sample}_guide{ref}_v{intron}_Operons_t{threshold}.clean.gtf",
            specie=config["specie"],
            sample=SAMPLES,
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
        ...
        expand("GAMBA_results/{specie}/{specie}_{sample}_guide{ref}_v{intron}_OperonGenes_t{threshold}.clean.gtf",
            specie=config["specie"],
            sample=SAMPLES,
            ref=config["stringtie_guide_opts"],
            intron=config["minimap2_max_intron"],
